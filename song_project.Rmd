...
for(i in 1960:2009){
  ...
  while(length(data_found_true$X) < 25){
    j <- j + 1
    #accept missing duration metadata info, but not missing lyrics if need be
...
# in this particular set, some information was lost, so add in manually
...
```{r}
# clear workspace for future quantification of features
rm_list <- objects()
...
```{r}
# Scatterplot of duration data over time
...
```{r}
# group data by chart AND year charted and calculate mean for plot simplification and comparison
...
```{r}
# Scatterplot of proportion of repeated stanzas over time
...
```{r}
# group data by chart and year and calculate mean and sd repetition
...
# plot mean repetition against time and including each set's lm line for comparison of general trend
ggplot() +
...
knnFit
#knnFit method lets us know that 35 k will prove most accurate
library(class)
...
for(i in 1:100){
  ...
  # add data over five folds to table
  for(i in 1:5){
...
# encode fold as categorical variable
accuracy_table$fold <- as.factor(accuracy_table$fold)

# group data by method and fold and calculate mean accuracy
t <- accuracy_table %>% group_by(method, fold) %>%
  summarise(
    mean_accuracy = mean(accuracy)
  )

# gather accuracy scores and display statistical info
...
# plot evaluations of each model's accuracy
ggplot(data = t, aes(x = fold, y = mean_accuracy, colour = method, group = method)) +
  ...
  labs(
    ...
    subtitle = "K Nearest Neighbor, Naive-Bayes, Random Forest DT, K-SVM, Logistic Regression, SVM"
  )
...
```{r}
# choosing topic model with 120 topics
